    ...
    async function fetchResults() {
      ...
      try {
        const params = new URLSearchParams({
          q: appliedQuery,
          // One list for everything update-related: approved requests
          // (including ones with a saved update draft) and updates awaiting review.
          status: "APPROVED,UPDATE_REQUESTED",
          sortKey: "submittedAt",
          sortDir: "desc",
          page: String(page),
          limit: String(limit),
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-200 bg-slate-50">
                ...
                <th className="px-4 py-3 text-left text-xs font-semibold text-slate-600 uppercase tracking-wide whitespace-nowrap">{tr("col.requestType")}</th>
