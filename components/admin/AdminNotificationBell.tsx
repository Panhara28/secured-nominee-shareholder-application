              ...
              >
                ...
                <div className="min-w-0">
                  ...
                  <p className={cn("text-xs mt-0.5", style.textColor)}>
                    {t(NOTICE_KEYS.USER as Parameters<typeof t>[0])}
                  </p>
                </div>
              </Link>
            </li>
          ...
          <li key={`request-${r.id}`}>
            ...
            >
              ...
              <div className="min-w-0">
                ...
                <p className={cn("text-xs mt-0.5", style.textColor)}>
                  {t(
                    (NOTICE_KEYS[r.status] ??
                      NOTICE_KEYS.PENDING) as Parameters<typeof t>[0],
                  )}
                </p>
              </div>
            </Link>
    ...
    async function fetchAwaitingReview() {
      try {
        const fetchStatus = (status: string) => {
          ...
          return fetch(`/api/secured/admin/requests?${params.toString()}`).then(
        ...
        const fetchPendingUsers = () => {
          ...
          return fetch(`/api/secured/admin/users?${params.toString()}`).then(
            (res) => (res.ok ? res.json() : null),
          );
        };
        ...
        const requestRows: PendingRow[] = requestResults.flatMap((r) =>
          (r?.data ?? []).map((d: Omit<PendingRequestRow, "kind">) => ({
          ...
          for (const row of newRows) {
            ...
            if (!knownKeys.current.has(key)) {
              ...
              if (row.kind === "user") {
                ...
                const body = t2(
                  `status.${row.status}` as Parameters<typeof t2>[0],
                );
        ...
        setRows(newRows);
        setTotal(
          requestResults.reduce((sum, r) => sum + (r?.total ?? 0), 0) +
            (userResult?.total ?? 0),
        );
      } catch {
